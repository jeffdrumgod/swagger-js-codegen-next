...
class CodeGen {
	...
	getViewForSwagger2() {
		...
		_.forEach(swagger.paths, (api, path) => {
			...
			data.methods = data.methods.concat(
				this.getMethodsEndPoint(swagger, api, path, globalParams)
			);
			data.definitions = data.definitions.concat(
				this.getDefinitionsEndPoint(swagger, api, path, globalParams)
			);
		});
	...
	getDefinitionsEndPoint(swagger, api, path, globalParams) {
		let definitions = [];
	...
	getMethodsEndPoint(swagger, api, path, globalParams) {
		...
		_.forEach(api, (operation, method) => {
			...
			_.forEach(params, (parameter) => {
				...
				if (path.indexOf('banners') > -1) {
					console.log(path, parameter.camelCaseName, parameter.in)
				}
